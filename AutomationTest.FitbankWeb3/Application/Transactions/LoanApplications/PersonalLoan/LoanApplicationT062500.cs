...
namespace AutomationTest.FitbankWeb3.Application.Transactions.LoanApplications
{
    public class LoanApplicationT062500 : LoanApplication<ClientDataT062500>
    {
        ...
        public override async Task<ILoanApplicationResult> ApplyForLoanAsync(IPage page, LoanApplicationModel<ClientDataT062500> loanApplication)
        {
            ...
            await page.Locator(_locators.ApplicationPageT062500.LoanAmount).FillAsync(clientData.LoanAmount.ToString());
            await page.Locator(_locators.ApplicationPageT062500.LoanAmount).PressAsync("Enter");
            await page.Locator(_locators.DashboardPage.OK).WaitForAsync(new LocatorWaitForOptions
            {
                State = WaitForSelectorState.Visible
            });

            await page.Locator(_locators.ApplicationPageT062500.LoanInstallments).FillAsync(clientData.LoanInstallments.ToString());
            await page.Locator(_locators.ApplicationPageT062500.LoanInstallments).PressAsync("Enter");
            await page.Locator(_locators.DashboardPage.OK).WaitForAsync(new LocatorWaitForOptions
            {
                State = WaitForSelectorState.Visible
            });
